...
public class DesktopWorkspaceViewModelTests
{
    ...
    public void PlayerHands_AllowSelectingCardsForEverySeat_WithoutChangingCurrentTurnSelectionCount()
    {
        ...
        waitingCard.IsSelected = true;

        Assert.True(waitingCard.IsSelected);
        Assert.Equal(0, workspace.SelectedCurrentPlayerCardCount);
        Assert.True(workspace.ShowDrawActions);
        Assert.False(workspace.ShowSelectionPrompt);
    }
    ...
    [Fact]
    public void MainWindowXaml_UsesResponsiveWrapHandLayoutsAroundTheTable()
    {
        ...
        Assert.Matches(new Regex(
            "ItemsControl ItemsSource=\\\"\\{Binding TopSeatHand\\.Cards\\}\\\"[\\s\\S]*?ItemContainerStyle=\\\"\\{StaticResource OverlappedHorizontalHandItemStyle\\}\\\"[\\s\\S]*?<WrapPanel Orientation=\\\"Horizontal\\\"\\s*/>[\\s\\S]*?PlayableCardTemplate",
            RegexOptions.CultureInvariant),
            xaml);
        Assert.Matches(new Regex(
            "ItemsControl Grid\\.Row=\\\"1\\\"[\\s\\S]*?ItemContainerStyle=\\\"\\{StaticResource OverlappedVerticalHandItemStyle\\}\\\"[\\s\\S]*?ItemsSource=\\\"\\{Binding LeftSeatHand\\.Cards\\}\\\"[\\s\\S]*?<WrapPanel Orientation=\\\"Vertical\\\"\\s*/>[\\s\\S]*?PlayableCardTemplate",
            RegexOptions.CultureInvariant),
            xaml);
        Assert.Matches(new Regex(
            "ItemsControl Grid\\.Row=\\\"1\\\"[\\s\\S]*?ItemContainerStyle=\\\"\\{StaticResource OverlappedVerticalHandItemStyle\\}\\\"[\\s\\S]*?ItemsSource=\\\"\\{Binding RightSeatHand\\.Cards\\}\\\"[\\s\\S]*?<WrapPanel Orientation=\\\"Vertical\\\"\\s*/>[\\s\\S]*?PlayableCardTemplate",
            RegexOptions.CultureInvariant),
            xaml);
        Assert.Matches(new Regex(
            "ItemsControl ItemsSource=\\\"\\{Binding BottomSeatHand\\.Cards\\}\\\"[\\s\\S]*?ItemContainerStyle=\\\"\\{StaticResource OverlappedHorizontalHandItemStyle\\}\\\"[\\s\\S]*?<WrapPanel Orientation=\\\"Horizontal\\\"\\s*/>[\\s\\S]*?PlayableCardTemplate",
            RegexOptions.CultureInvariant),
            xaml);
    ...
    public void MainWindowXaml_ExpandsSideSeatHandsVertically()
    {
        ...
        Assert.Contains("<Setter Property=\"Padding\" Value=\"0\" />", xaml, StringComparison.Ordinal);
        Assert.DoesNotContain("<Setter Property=\"Background\" Value=\"#18243447\" />", xaml, StringComparison.Ordinal);
        ...
        Assert.Contains("<Grid Margin=\"0,0,10,0\">", leftSeatSection, StringComparison.Ordinal);
        ...
        Assert.Contains("PlayableCardTemplate", leftSeatSection, StringComparison.Ordinal);
        ...
        Assert.Contains("<Grid Margin=\"10,0,0,0\">", rightSeatSection, StringComparison.Ordinal);
        ...
        Assert.Contains("PlayableCardTemplate", rightSeatSection, StringComparison.Ordinal);
    ...
    public void MainWindowXaml_CentersStockAndDiscardPilesTogether()
    {
        ...
        Assert.Matches(new Regex(
            "<Border Grid\\.Row=\\\"1\\\"[\\s\\S]*?Grid\\.Column=\\\"2\\\"[\\s\\S]*?Background=\\\"#1A17212B\\\"",
            RegexOptions.CultureInvariant),
            xaml);
        Assert.DoesNotContain("CanastaNET table", xaml, StringComparison.Ordinal);
        Assert.DoesNotContain("Header=\"Table actions\"", xaml, StringComparison.Ordinal);
        Assert.DoesNotContain("Header=\"Table state\"", xaml, StringComparison.Ordinal);
        Assert.DoesNotContain("Header=\"Team melds\"", xaml, StringComparison.Ordinal);
        Assert.DoesNotContain("Header=\"Legal action summary\"", xaml, StringComparison.Ordinal);
        Assert.DoesNotContain("Header=\"Score summary\"", xaml, StringComparison.Ordinal);
        Assert.Contains("automation:AutomationProperties.Name=\"Stock pile\"", xaml, StringComparison.Ordinal);
        Assert.Contains("automation:AutomationProperties.Name=\"Discard pile\"", xaml, StringComparison.Ordinal);
        Assert.Contains("automation:AutomationProperties.HelpText=\"Face-down draw pile.\"", xaml, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("Text=\"Central piles\"", xaml, StringComparison.Ordinal);
